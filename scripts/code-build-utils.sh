...
function packagePythonLambdas () {
  ...
  for d in */; do
    ...
    if [ "${d}" = "__pycache__" ]; then
      ...
      sha256sum ${d}.zip | cut -f1 -d\ | xxd -p | base64 > ${d}.zip.hash
...
function syncZipsAndHashesToS3 () {
  ...
  local S3_URI="s3://${BUCKET_NAME}/${PROJECT_NAME}/${CODEBUILD_SOURCE_VERSION}"
  ...
  aws s3 sync ${CODEBUILD_SOURCE_VERSION}/ ${S3_URI} --exclude "*" --include "*.zip.hash" --sse aws:kms --sse-kms-key-id ${KMS_KEY_ALIAS} --content-type "text/plain"
}
